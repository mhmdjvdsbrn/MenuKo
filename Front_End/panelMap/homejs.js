		...
		ajaxerPagerEdit = $.ajax({
			...
			success: function(response){
				...
				$(".submit_edit").click(function(){
					...
					var text = $("#txt_matn").val();
					var name = $("#txt_name").val();
        			var address = $("#txt_address").val();
					var numPhone = $("#txt_numPhone").val();
					var categoryId = $('#loadinfo_select_raste').val();
					var city = $('#loadinfo_select_city').val();
				    ...
				    if(check){
    					...
    					$.ajax({
    						...
    						type: 'post',
    						data: {id:id,text:text,name:name,address:address,numPhone:numPhone,categoryId:categoryId,city:city},
    						dataType: 'JSON',
    						success : function(response){	
...
function delInfo(str,name,family){
	...
	document.getElementById("delete_alertDialog").innerHTML = "<center><div class='text'><br>آیا مطمعن هستید که میخواهید کاربر "+name + " " + family + " " + "را حذف کنید؟</div><div dir='ltr' style='display:inline-flex'><div class='btn btn_submit' onclick='del_submited("+ str +")'><div class='btn_effect'></div><div class='text' style='position:relative;z-index:2'>بله</div></div><div class='btn btn_deny' onclick='deny_delete()'><div class='btn_effect'></div><div class='text'  style='position:relative;z-index:2'>خیر</div></div></div>";
}
    ...
    $("document").ready(function(){
        ...
        $.ajax({
            ...
            error: function() {
                // re call ajax
                clearAllTimeOut();
                timeouts.push(setTimeout(function(){del_submited(id)}, 1200));
            }
        });
    });
}
...
function search_pak(type){
    ...
    $("document").ready(function(){
        $('#search_select_namayesh').val('');
        $('#search_select_pardakht').val('');
...
function addUserAlert(mapAddress,lat,lang){
	...
	notif_alert_manager('','','get_field_add_maghaze_alertDialog','');
	
	$("#get_field_add_maghaze_alertDialog").html("<div class='loader'></div>");
	...
	$.ajax({
		...
		success: function(response){
		    ...
		    $("#get_field_add_maghaze_alertDialog").html(response);
...
function startTime(temp) {
    ...
    m = checkTime(m);
    s = checkTime(s);
    document.getElementById('writerSaat').innerHTML = h + ":" + m + ":" + s;
    document.getElementById('writerPM_AM').innerHTML = ampm;
    ctoday += 1000;
}
setInterval(function(){startTime(ctoday)},1000);
