...
function deny_add_pic(){
    notif_alert_manager('','1','','اضافه کردن محصول لغو شد');
}
			...
			img.onload = function() {
                ...
                if(uploadFor != 'aza')
                    if(this.width < 200 || this.height < 200){
            		    $("#notification").find(".text").html('حداقل عرض تصویر 200px و حداقل طول تصویر 200px باید باشد');
...
function raste_manage(){
	...
	$("document").ready(function(){
	    ...
	    $.ajax({
        	url: '../api/load_category_ajaxer.php',
        	type: 'post',
        	data: {token:token},
        	dataType: 'HTML',
        	success: function(response){
...
function add_raste_sabt(){
    
    var rasteTitle = $("#txt_add_raste").val();
    var rasteIsikcode = $("#txt_add_isikcode").val();
    ...
    if(rasteIsikcode == ''){
        check = false;
        notif_alert_manager('','','','کد آیسیک خالی است');
    }
    else{
        if(isNaN(rasteIsikcode)){
            check = false;
            notif_alert_manager('','','','کد آیسیک باید عدد باشد');
        }
    }
    ...
    if(check){
        if(rasteTitle == ''){
            check = false;
            notif_alert_manager('','','','عنوان رسته خالی است');
        }
    }
	
	if(check){
        $.ajax({
        	url: 'add_raste_ajaxer.php',
        	type: 'post',
        	data: {tb_year:tb_year,rasteTitle:rasteTitle,rasteIsikcode:rasteIsikcode},
        	dataType: 'HTML',
        	success: function(response){
