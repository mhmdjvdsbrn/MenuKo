<?php
    ...
    $url = 'http://menuko.ir:7000/api/menu/';
    			...
    			for($a = 0;$a < $countCategories;$a++){
                    ...
                    for($i = 0;$i < $countProducts;$i++){
            			...
            			<div class="ticket_box_score">
            			    ...
            			    <div class="linker box_delete" id="close_print_alert" onclick="delInfo('."'".$response2[$a]['products'][$i]['id']."','".$response2[$a]['products'][$i]['title']."','مجیدی'".')">
                            		<img src="../delete.svg" width="18px" height="18px">
                            </div>
                            
                            ';
                            
                            
                            $mohtava = $response2[$a]['products'][$i]['ingredients'];
                            $mohtava = str_replace('"','',$mohtava);
                            $mohtava = str_replace('"','',$mohtava);
                            $mohtava = str_replace('\n','',$mohtava);
                            $mohtava = trim(preg_replace('/\s+/', ' ', $mohtava));


                            echo '
                            
                            <div class="linker box_delete" style="left:36px" id="close_print_alert" onclick="editProductInfo('."'".$response2[$a]['products'][$i]['id']."','".$response2[$a]['products'][$i]['title']."','".$response2[$a]['category_name']."','".$mohtava."','".$response2[$a]['products'][$i]['price']."'".')">
                            		<img src="../edit.svg" width="18px" height="18px">
                            </div>
